...
function startButton(graph) {
  ...
  startButton.style.fontSize = startButton.clientHeight * 0.7 + "px";
...
function resetButton() {
  ...
  resetButton.style.fontSize = resetButton.clientHeight * 0.7 + "px";
...
function tickButtons(allGraphs) { 
  for(i = -1; i < 2; i+=2) {
    ...
    tickButton.style.fontSize = tickButton.clientHeight * 0.7 + "px";
  }
}
...
function forwardTick(storedGraphs) {
  let emptyPath = true;
  ...
  for ([key, value] of Object.entries(baseGraph)) {
    ...
    if(colorNames[key]) {
      ...
      if(key != `white`) {
        ...
        newEntrance.style.left = endPipe.offsetLeft + endPipe.parentElement.offsetLeft + "px";
        newEntrance.style.top = endPipe.offsetTop + endPipe.parentElement.offsetTop + newEntrance.clientHeight * 3 + "px";   
      }
      continue;
    }
    ...
    nextPosition.children[0].src = `./images/${pipeType}.png`;
...
function resetLiquids() {
  ...
  Array.from(document.querySelectorAll(`.filledContainer`)).forEach((container) => {
    container.remove();
  });
  pastPaths = {
    cyan: [],
...
function liquidStep(direction) {
  ...
  if(direction == `back`) {
    ...
    if(endPipe.liquidBox) {
      endPipe.liquidBox.remove();
      Array.from(document.querySelectorAll(`.filledContainer`)).forEach((oldContainer) => {
        if(oldContainer.liquidPosition == Array.from(document.querySelectorAll(`.filledContainer`)).length) {
          endPipe.liquidBox = oldContainer;
        }
      });
    }
  } else {
    //start of trying to seperate merged colors to allow bypass
    //getKeyByArray(colorNames, document.getElementById(`${activePaths[path][0]}, placed`).lastChild.currentColor)
    // ^ gets the current color's name of each pipe on the active path i think?
    // then maybe re-shortest path from that pipe to end pipe using that color's graph?
    //then disconnect that specific color from the active path but also decrease the active paths of that colors colors???
    ...
    if(endPipeLastColor) {
      let container = document.getElementById(`${endPipe.id.replace(`, placed`, ``)}/white`);
      let boxHolder = document.getElementById(`boxHolder`);
      let newContainer = container.cloneNode(true);
      boxHolder.appendChild(newContainer);
      let containerColor = endPipeLastColor;
      newContainer.classList.remove(`liquidBox`);
      newContainer.style.visibility = `visible`;
      newContainer.style.backgroundColor = `rgb(${containerColor[0]}, ${containerColor[1]}, ${containerColor[2]})`;
      newContainer.classList.add(`filledContainer`);
      newContainer.id = containerColor;
      newContainer.liquidPosition = Array.from(document.querySelectorAll(`.filledContainer`)).length;
      endPipe.liquidBox = newContainer;
      if(!debugState) {
        let placeHolder = conveyorToHolder(newContainer);
        placeHolder.liquidPosition = Array.from(document.querySelectorAll(`.filledContainer`)).length;
        endPipe.liquidBox = placeHolder;
        setTimeout(replaceBoxDiv, tickSpeed, newContainer, placeHolder);
      }
    }
  }
  Array.from(document.querySelectorAll(`.pipe`)).forEach((pipe) => {
  ...
  for(let path of Object.keys(futurePaths)) {
    for (let pipeID of activePaths[path]) {
      ...
      if(pipe) {
        ...
        let result = solver.solve();
        let filterCSS = result.filter;
        pipe.lastChild.style.filter = `opacity(75%) brightness(0%) ` + filterCSS.replace(`filter: `, ``).replace(`;`,``);
      }
    }
  }
  if(continueTimeout) {
...
function createIntersection(nodeDiv, neighborDiv, number) {
  ...
  newIntersection.style.width = squareSize / 3 + "px";
  newIntersection.style.height = squareSize / 3 + "px";
  ...
  addImg(imageNumber, newIntersection, (newDiv) => {
    ...
    newDiv.style.height = `${newIntersection.clientHeight}px`;
  });
...
function generatePipes(fromGraph) {
  if (!pipesContainer) {
    ...
    pipesContainer.style.top = (window.innerHeight / 2) - (pipesContainer.clientHeight / 2) + "px";
...
function dragElement(elmnt) {
  ...
  // otherwise, move the DIV from anywhere inside the DIV:
    let debugItem = Array.from(elmnt.classList).find((value) => {
      return value.includes(`debug`);
    });
    if(debugItem) {
  ...
  function copyAndDrag(event) {
    ...
    let newPieceClass = Array.from(this.classList).find((value) => {
      return value.includes(`debug`);
    });
    newPiece.classList.add(`${newPieceClass.replace(`debug`,`draggable`)}`);
    newPiece.classList.remove(newPieceClass);
    if (Array.from(newPiece.classList).includes(`draggablePipe`)) {
      newPiece.pipeType = this.pipeType;
      newPiece.defaultConnections = this.defaultConnections;
      newPiece.rotation = 0;
      newPiece.style.width = squareSize + "px";
      newPiece.style.height = squareSize + "px";
    }
    else if (Array.from(newPiece.classList).includes(`draggableIntersection`)) {
      newPiece.style.width = squareSize / 3 + "px";
      newPiece.style.height = squareSize / 3 + "px";
    }
    else if(Array.from(newPiece.classList).includes(`draggableColorFilter`)) {
      newPiece.style.width = (squareSize * .75) + "px";
      newPiece.style.height = squareSize / 4 + "px";
    } else if(Array.from(newPiece.classList).includes(`draggableEntrance`)) {
      newPiece.style.width = squareSize + "px";
      newPiece.style.height = squareSize / 2 + "px";
    }
    newPiece.classList.add(`draggableElement`);
    newPiece.style.left = this.parentElement.offsetLeft + this.offsetLeft + "px";
    newPiece.style.top = this.parentElement.offsetTop + this.offsetTop + "px";
    dragElement(newPiece);
    elmnt = newPiece;
    elmnt.numberID = this.numberID;
    elmnt.color = this.color;
    dragMouseDown(event);
  }
